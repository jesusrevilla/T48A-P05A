...
serie_numeros = crear_serie([1,2,3])
print(serie_numeros.tolist())
...
def crear_dataframe(diccionario):
    ...
    return df
diccionario = {'a': [1, 2], 'b': [3, 4]}
df = crear_dataframe(diccionario)
print(df)
...
df = pd.DataFrame({'x': [10, 20], 'y': [30, 40]})
columna_seleccionada = seleccionar_columna(df, 'y')
print(columna_seleccionada.tolist())
...
df = pd.DataFrame({'edad': [15, 25, 35]})
resultado = filtrar_por_condicion(df, 'edad', 20)
print(resultado)
...
def crear_indexacion_jerarquica():
    ...
    np.int64(1)
    """
    serie = pd.Series(range(1, 5), index=[['a', 'a', 'b', 'b'], [1, 2, 1, 2]])
...
def agrupar_y_promediar(df, columna_agrupacion, columna_valores):
    ...
    True
    """
    df_agrupado = df.groupby(columna_agrupacion)[columna_valores].mean()
    return df_agrupado
...
def combinar_dataframes(df1, df2, clave):
    ...
    (2, 3)
    """
    df_combinado = pd.merge(df1, df2, on=clave)
    return df_combinado
